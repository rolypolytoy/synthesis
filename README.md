...
A gate-level netlist is a toy level model that I can see, and since it's plaintext it's a lot easier to verify if the gates are there as they're expected to be, than if it was a file format I was personally unfamiliar with and harder to manipulate. Modify the synth.ys file if you want functional files, but you'll probably have to add some additional lines of code depending on what process you use. Netlists are just easier.
